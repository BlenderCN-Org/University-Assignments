...
SELECT T.Name, T.PersonId
  FROM (SELECT P.Name, P.PersonId, COUNT(DH.DepartmentId) AS DEPT_COUNT
          FROM Person P, Department_History DH
         WHERE (P.PersonId = DH.EmployeeId) GROUP BY P.Name, P.PersonId) T,
         (SELECT COUNT(D.DepartmentId) AS DEPT_MAX FROM Department D) X
 WHERE (T.DEPT_COUNT = X.DEPT_MAX);

--<Query 12>
SELECT P.Name, P.Email
  FROM PERSON P, (SELECT PERSONID, JOBID, SUM(GRADE) / COUNT(ROUNDNUMBER) AS AVERAGE_SCORE, COUNT(ROUNDNUMBER) AS ROUNDCOUNT
                    FROM (SELECT DISTINCT P.PERSONID, IG.ROUNDNUMBER, IG.GRADE, I.JobId
...
--SELECT PERSONID, JOBID, SUM(GRADE) / COUNT(ROUNDNUMBER) AS AVERAGE_SCORE, COUNT(ROUNDNUMBER) AS ROUNDCOUNT
--                      FROM (SELECT DISTINCT P.PERSONID, P.NAME, P.EMAIL, IG.ROUNDNUMBER, IG.GRADE, I.JobId
--                              FROM JOB J, PERSON P, POTENTIALEMPLOYEE PE, INTERVIEW_GRADE IG, INTERVIEW I
--                             WHERE (IG.INTERVIEWID = I.INTERVIEWID AND I.APPLICANTID = PE.POTENTIALEMPLOYEEID AND I.APPLICANTID = P.PERSONID)) group by JOBID, PERSONID;
--                             
--SELECT B.PERSONID, COUNT(B.JOBID) FROM (SELECT DISTINCT P.PERSONID, I.JobId
--                      FROM PERSON P, POTENTIALEMPLOYEE PE, INTERVIEW_GRADE IG, INTERVIEW I
--                     WHERE (IG.INTERVIEWID = I.INTERVIEWID AND I.APPLICANTID = PE.POTENTIALEMPLOYEEID AND I.APPLICANTID = P.PERSONID)) B group by B.PERSONID;

SELECT G.NAME, G.Phone, G.Email
  FROM (SELECT P.Name, P.Phone, P.Email, P.PersonId, COUNT(X.JOBID) AS JOB_COUNT
    FROM PERSON P, (SELECT PERSONID, JOBID, SUM(GRADE) / COUNT(ROUNDNUMBER) AS AVERAGE_SCORE, COUNT(ROUNDNUMBER) AS ROUNDCOUNT
                      FROM (SELECT DISTINCT P.PERSONID, P.NAME, P.EMAIL, IG.ROUNDNUMBER, IG.GRADE, I.JobId
                              FROM JOB J, PERSON P, POTENTIALEMPLOYEE PE, INTERVIEW_GRADE IG, INTERVIEW I
                             WHERE (IG.INTERVIEWID = I.INTERVIEWID AND I.APPLICANTID = PE.POTENTIALEMPLOYEEID AND I.APPLICANTID = P.PERSONID)) group by JOBID, PERSONID)  X
   WHERE (X.ROUNDCOUNT >=5 AND X.AVERAGE_SCORE >= 70 AND X.PERSONID = P.PERSONID) GROUP BY P.Name, P.Email, P.PersonId, P.Phone) G,
                   (SELECT B.PERSONID, COUNT(B.JOBID) AS JOB_COUNT FROM (SELECT DISTINCT P.PERSONID, I.JobId
                      FROM PERSON P, POTENTIALEMPLOYEE PE, INTERVIEW_GRADE IG, INTERVIEW I
                     WHERE (IG.INTERVIEWID = I.INTERVIEWID AND I.APPLICANTID = PE.POTENTIALEMPLOYEEID AND I.APPLICANTID = P.PERSONID)) B group by B.PERSONID) X
 WHERE (X.PERSONID = G.PERSONID AND G.JOB_COUNT = X.JOB_COUNT);
