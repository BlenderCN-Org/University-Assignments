# AUTHOR:: MATTHEW G MCMILLIAN
# STUDENT_ID:: mgm160130
...
#	-data.txt
#
# This project creates a file 'data.txt',
# fills the file in this formater: abcd\nefgh\n...
# where each letter is a random value of 0 or 1. 0 indicating a loss, and 1 indicating a win
# The program will then read the file it just created, sum up each column's total, and create a bar graph based on the data
# The project report details the scenario this has *it has a story*
	
	.data
# Name of the file we are doing file I/O on
filename:	.asciiz	"data.txt"
# Hexcode for NEWLINE (Used in FILE I/O because of MARS issue
newline:	.asciiz "0xD"
...
ravenprompt:	.asciiz "\nRavenclaw:: "
hufflprompt:	.asciiz "\nHufflepuff:: "
gryphprompt:	.asciiz "Gryffindor:: "
scoreprompt:	.asciiz "::SCOREBOARD::\n"

# Hardcoded .asciiz of '1'
one:		.asciiz "1"
# Hardcoded .asciiz of '0'
zero:		.asciiz "0"
...
buffer:		.space 2
		...
		.align	2
# tmp is used to fix a wierd problem involving the buffer and spacing
...
white:		.word	0xFFF0FFFF
		...
		.align 2
# Starting address for the bitmap editor
...
create_vert_graph: 	
	...
	# Outprints the total score Gryffindor
	li $v0, 4
	la $a0, gryphprompt
	syscall	
	li $v0, 1
	...
	# Outprints the total score Hufflepuff
	li $v0, 4
	la $a0, hufflprompt
	syscall	
	li $v0, 1
	...
	# Outprints the total score Ravenclaw
	li $v0, 4
	la $a0, ravenprompt
	syscall	
	li $v0, 1
	...
	# Outprints the total score Slytherinn
	li $v0, 4
	la $a0, slythprompt
	syscall	
	li $v0, 1
